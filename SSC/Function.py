...
class Function(ABC):
    def __init__(self) -> None:
        self.__func_log = getLogger('SSC.function')
    ...
    def execute(self, topic_crt : TopicsCrt, timeout : int):
        self.__func_log.info(f'function thread start... {self.name}')
        ...
        while (self.alive):
            ...
            if (self.topic_in) and (self.topic_in.qsize() > 0):
                ...
                if res:
                    ...
                    self.__func_log.debug(f'Function exec {self.name} topic in: {self.topic_in.name} ..')
                    inputs += 1

                    try:

                        ret = self.process(res, Context(topic_crt, self.__func_log))
                        ...
                        self.tot_erro += 1
                        self.__func_log.error(f'Function exec {self.name} erro: ' + exp.args[0])
                        time.sleep(1)

                    if (self.topic_out) and (ret != None):

                        self.__func_log.debug(f'Function exec {self.name} topic out: {self.topic_out.name} ..')
        ...
        self.__func_log.info(f'function thread stop... {self.name}')